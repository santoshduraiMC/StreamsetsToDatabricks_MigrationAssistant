...
SECRET_TOKEN    = _load_secret("DATABRICKS_TOKEN", "")
SECRET_BASE_URL = _load_secret("BASE_URL", "https://adb-xxxx.azuredatabricks.net/serving-endpoints")
SECRET_MODEL    = _load_secret("MODEL_NAME", "databricks-claude-sonnet-4")

# === Settings state ===
if "cfg" not in st.session_state:
    st.session_state.cfg = {
        "use_overrides": False,               # controls Token & Base URL only
        "token_override": "",
        "base_url_override": SECRET_BASE_URL,
        "model_name": SECRET_MODEL,           # always editable
    }

def _effective(secret_val: str, override_val: str, enabled: bool) -> str:
    return (override_val or secret_val) if enabled else secret_val

# === Settings button at top-right ===
col_title, col_settings = st.columns([10, 1])
with col_settings:
    try:
        settings = st.popover("⚙️", use_container_width=True)
    except Exception:
        settings = st.expander("⚙️ Settings", expanded=False)

with settings:
    st.markdown("###### Inference Settings")

    st.toggle(
        "Use overrides for Token & Base URL",
        key="cfg_use_overrides",
        value=st.session_state.cfg["use_overrides"],
    )
    st.session_state.cfg["use_overrides"] = st.session_state.get("cfg_use_overrides", False)

    st.text_input(
        "Serving Base URL",
        key="cfg_base_url_override",
        value=st.session_state.cfg["base_url_override"],
        help="Your Databricks Serving Endpoints root",
        disabled=not st.session_state.cfg["use_overrides"],
    )

    if st.session_state.cfg["use_overrides"]:
        st.text_input(
            "Databricks Token (override, not stored)",
            key="cfg_token_override",
            type="password",
            value=st.session_state.cfg["token_override"],
            help="Leave blank to keep using the secret token",
        )
        st.session_state.cfg["token_override"] = st.session_state.get("cfg_token_override", "")
    else:
        st.info("Using token from Secrets (hidden).")

    # Model name is always editable (defaults from secrets)
    st.text_input(
        "Serving Endpoint Name",
        key="cfg_model_name",
        value=st.session_state.cfg.get("model_name", SECRET_MODEL),
        help="Endpoint to call (e.g., databricks-claude-sonnet-4)",
    )

# Compute effective configuration
use_overrides = st.session_state.cfg["use_overrides"]
token_effective = _effective(SECRET_TOKEN, st.session_state.cfg["token_override"], use_overrides).strip()
base_url        = _effective(SECRET_BASE_URL, st.session_state.cfg["base_url_override"], use_overrides).strip()
model_name      = st.session_state.cfg.get("cfg_model_name", SECRET_MODEL).strip()

# Build client
client = OpenAI(api_key=token_effective, base_url=base_url) if token_effective else None

# === Sidebar: Stages ===
with st.sidebar:
    st.header(" Migration Stages")
    stage = st.radio("📌 Select Stage", ["Stage 1: Parse & Visualize", "Stage 2: Databricks Alignment", "Stage 3: Generate Notebook"])

# Warn if no token
if not client:
    st.warning("No token available. Add DATABRICKS_TOKEN to secrets.toml or enable overrides in ⚙️ Settings.")
            ...
            with st.spinner("🧠 Migration Assistant is parsing and visualizing..."):
                try:
                    resp = client.chat.completions.create(
                        model=model_name, max_tokens=10000, temperature=0.3,
...
""")
    ...
    if st.session_state.stage1_output_visible:
        with st.expander("🔎 View Stage 1 Summary", expanded=False):
            st.text_area("Stage 1 Result", st.session_state.stage1_output_visible, height=260, disabled=True)
        ...
        if not st.session_state.json_content:
            ...
            with st.spinner("🧠 Migration Assistant is aligning with Databricks..."):
                try:
                    ...
                    stage2_output = resp.choices[0].message.content
        ...
        if not st.session_state.json_content:
            ...
            with st.spinner("🛠️ Migration Assistant is generating notebook..."):
                try:
                    ...
                    nb = resp.choices[0].message.content
